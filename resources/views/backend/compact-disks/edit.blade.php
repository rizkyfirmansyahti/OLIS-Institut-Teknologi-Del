...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <form action="{{ route('backend.compact-disks.update', encodeId($compact_disk->id)) }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="card-header">
                            <h3 class="card-title">Edit CD/DVD</h3>
                        </div>
                        <!-- /.card-header -->
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('title') is-invalid @enderror"
                                    id="title" name="title" value="{{ old('title', $compact_disk->title) }}">
                                @error('title')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('subject') is-invalid @enderror"
                                    id="subject" name="subject" value="{{ old('subject', $compact_disk->subject) }}">
                                @error('subject')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('author') is-invalid @enderror"
                                    id="author" name="author" value="{{ old('author', $compact_disk->author) }}">
                                @error('author')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description">{{ old('description', $compact_disk->description) }}</textarea>
                                @error('description')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('source') is-invalid @enderror"
                                    id="source" name="source" value="{{ old('source', $compact_disk->source) }}">
                                @error('source')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control @error('major') is-invalid @enderror" id="major"
                                    ...
                                    <option value="D3 Teknologi Komputer"
                                        @if (old('major', $compact_disk->major) == 'D3 Teknologi Komputer') selected @endif>D3
                                        Teknologi Komputer</option>
                                    <option value="D3 Teknologi Informasi"
                                        @if (old('major', $compact_disk->major) == 'D3 Teknologi Informasi') selected @endif>D3
                                        Teknologi Informasi</option>
                                    <option value="D4 Teknologi Rekayasa Perangkat Lunak"
                                        @if (old('major', $compact_disk->major) == 'D4 Teknologi Rekayasa Perangkat Lunak') selected @endif>D4
                                        Teknologi Rekayasa Perangkat Lunak</option>
                                    <option value="S1 Informatika" @if (old('major', $compact_disk->major) == 'S1 Informatika') selected @endif>
                                        S1 Informatika</option>
                                    <option value="S1 Sistem Informasi" @if (old('major', $compact_disk->major) == 'S1 Sistem Informasi') selected @endif>
                                        S1 Sistem Informasi</option>
                                    <option value="S1 Teknik Elektro" @if (old('major', $compact_disk->major) == 'S1 Teknik Elektro') selected @endif>
                                        S1 Teknik Elektro</option>
                                    <option value="S1 Manajemen Rekayasa"
                                        @if (old('major', $compact_disk->major) == 'S1 Manajemen Rekayasa') selected @endif>
                                        S1 Manajemen Rekayasa</option>
                                    <option value="S1 Teknik Metalurgi" @if (old('major', $compact_disk->major) == 'S1 Teknik Metalurgi') selected @endif>
                                        S1 Teknik Metalurgi</option>
                                    <option value="S1 Teknik Bioproses" @if (old('major', $compact_disk->major) == 'S1 Teknik Bioproses') selected @endif>
                                        S1 Teknik Bioproses</option>
                                    <option value="Umum/Lainnya" @if (old('major', $compact_disk->major) == 'Umum/Lainnya') selected @endif>
                                        Umum/Lainnya</option>
                                </select>
                                @error('major')
                                    <div class="text-danger">{{ $message }}</div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('year') is-invalid @enderror"
                                    id="year" name="year" value="{{ old('year', $compact_disk->year) }}">
                                @error('year')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
